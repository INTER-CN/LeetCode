...
public class SpiralOrder {
    ...
    public List<Integer> spiralOrder2(int[][] matrix) {
        ...
        spiralOrder2(result, matrix, 0, 0, m - 1, 0, n - 1);

        return result;
    }

    private void spiralOrder2(List<Integer> result, int[][] matrix, int index, int x1, int x2, int y1, int y2) {
        if (x1 > x2 || y1 > y2) return;
        ...
        if (y1 == y2) {
            for (int i = x1; i <= x2; i++) result.add(matrix[i][y1]);
            return;
        }

        for (int i = y1; i < y2; i++) result.add(matrix[x1][i]);
        for (int i = x1; i < x2; i++) result.add(matrix[i][y2]);
        for (int i = y2; i > y1; i--) result.add(matrix[x2][i]);
        for (int i = x2; i > x1; i--) result.add(matrix[i][y1]);

        spiralOrder2(result, matrix, index, x1 + 1, x2 - 1, y1 + 1, y2 - 1);
    }
