...
public class RemoveComments {
    ...
    public List<String> removeComments(String[] source) {
        ...
        String s = String.join(System.lineSeparator(), source);
        int index = 0;
        char c1, c2;
        while (index < s.length() - 1) {
            ...
            if (c1 == '/' && c2 == '/') {
                index += 2;
                while (index < s.length() && !s.substring(index).startsWith(System.lineSeparator())) index++;
        ...
        if (index == s.length() - 1) builder.append(s.charAt(index));

        String[] lines = builder.toString().split(System.lineSeparator());

        return Arrays.asList(lines).stream().filter(line -> line.length() > 0).collect(Collectors.toList());
    }
...
//            "}"
//        };
        List<String> result = new RemoveComments().removeComments(source);
